...
**v1.3 で変更(2026-09-19)**: iPhone で a[download] がファイルアプリに入るというユーザー所見を受け、スマホは共有メニューを主経路にした。

- スマホ(`pointer: coarse` かつファイル共有可): 「保存する」→ 書き出し後に `navigator.share({files:[file]})` を直接呼ぶ。iPhone はここで「画像を保存」を選ぶと写真アプリに入る。`title` は付けない
  - NotAllowedError(書き出しが長くユーザー操作の有効期限が切れた)などで失敗したら、下のダイアログを出す。AbortError(利用者が閉じた)なら何も出さない
  - ダイアログは `[ファイルに保存] [閉じる] [写真に保存(主)]` と説明文。「写真に保存」の click 内で share を呼び直す
- パソコン: ダイアログ `[共有する] [閉じる] [ファイルとして保存(主)]`
...
## 8. v1.2 光学フィルター箱(2026-09-18 追加)

ユーザーが claude.ai で相談した案のうち、ブラックミスト・ハレーション・アナモルフィック風フレアを同じツールのモードとして追加した。重ねがけ可。既定はすべて 0(オフ)。

| モード | 元 | 広げ方 | 合成 | スライダー |
|---|---|---|---|---|
| ブラックミスト | hl.rgb = 線形色 × smoothstep(0.35, 0.70, v)(v = luma と max の中間) | hl のミップを lod, +1, +2 で 0.5/0.3/0.2 加重。lod = log2(長辺/(96−84r)) | add += mist × amt、amt = (m/100)^1.5 × 1.2 | 強さ、広がり |
| ハレーション | hl.a = 線形輝度 × smoothstep(0.73, 0.89, v) | 同上。lod = log2(長辺/(160−120r)) | add += tint(1.0, 0.32, 0.08) × hal × amt、amt = (h/100)^1.3 × 2.0 | 強さ、広がり |
| アナモルフィック | c(点光源の重み付き明部)を lod 1.0 で読む | 既存の連鎖を水平 2 方向・半解像度で。τ = (0.005 + 0.12·L^1.6)·長辺/2 | add += tint(0.45, 0.65, 1.0) × luma(ana − 2·c) × amt/rel、amt = (a/100)^1.3 × 0.6 | 強さ、長さ |

テクスチャ追加: `hl`(work 解像度、RGBA16F、mips)、`ana`(半解像度)。hl の描画とミップ生成はミストかハレーションが有効なときだけ。アナモルフィックの連鎖も有効なときだけ。
UI: 「こまかい調整」の下に details を 3 つ。見出しに有効中の強さをバッジで出す。localStorage キーは v4 のまま(既存スライダーの意味は不変で、無いキーは既定 0 になる)。
検証: `tests/matrix.cjs` が `matrix_fx_v1.png`(なし / ミスト / ハレーション / アナモルフィック / 3 つ重ね / ミストのみ)を出す。既存の条件 1〜6 はすべて既定(オフ)で通過することを確認。
